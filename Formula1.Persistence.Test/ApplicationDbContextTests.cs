...
namespace Formula1.PersistenceTests
{
  ...
  public class ApplicationDbContextTests
  {
    ...
    public async Task D01_FirstDataAccessTest()
    {
      ...
      [TestMethod]
      public async Task D02_SecondDataAccessTest()
      {
         string dbName = Guid.NewGuid().ToString();
         ...
         using (ApplicationDbContext dbContext = GetDbContext(dbName))
         {
            ...
            var lastOrDefault = dbContext.Teams.LastOrDefault(t => t.Name == "Mercedes");
            Assert.AreEqual(2, dbContext.Teams.Count());
            Assert.AreEqual("Mercedes",lastOrDefault.Name);
         }
      }
   }
}